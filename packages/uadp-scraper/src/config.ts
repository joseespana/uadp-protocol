...
export const config = {
  ...
  limits: {
    ...
    videos: parseInt(process.env.SCRAPE_VIDEOS_MAX ?? "100", 10),
  ...
  rss: {
    ...
    travel: (process.env.RSS_TRAVEL ?? "").split(",").filter(Boolean),
    sports: (process.env.RSS_SPORTS ?? "").split(",").filter(Boolean),
    entertainment: (process.env.RSS_ENTERTAINMENT ?? "").split(",").filter(Boolean),
    lifestyle: (process.env.RSS_LIFESTYLE ?? "").split(",").filter(Boolean),
    gaming: (process.env.RSS_GAMING ?? "").split(",").filter(Boolean),
  },
